...
This Streamlit app builds upon Sage's dashboard template. We have a "main" funder-centric dashboard vs more "internal" dashboard. Read more on each below and see ## Set up this Streamlit application

### Main Dashboard

The main funder-facing dashboard: `streamlit run app.py`.

### Internal Dashboard

The internal dashboard (`internal-app.py`) is specifically designed for DCC staff to monitor project compliance and administrative metrics. This dashboard is separate from the main funder-facing dashboard to maintain clear separation of concerns and security.

### Features
- **Project Compliance Monitoring**
  - Track missing or unclear PI information
  - Monitor institution affiliations
  - Export data for follow-up actions

- **Access Requirements Overview**
  - View and monitor access conditions across all projects
  - Track DUC, IRB, and IDU requirements
  - Monitor certification requirements
  - Export compliance data for reporting

### Running the Internal Dashboard
```bash
streamlit run internal-app.py
```

> [!IMPORTANT]  
> The internal dashboard contains sensitive administrative information. Ensure you have appropriate permissions before accessing it.

### Shared Dashboard Components
The internal dashboard shares common components with the main app:
- Utility functions and database connections
- Staff user ID lists
- Styling and configuration
- Query toolkit

This structure allows for easy maintenance while keeping the option open to split into separate repositories in the future if needed.
...
## Set up this Streamlit application

### NF-specific steps and tips

> [!IMPORTANT]  
> Before following the general setup instructions below to run an instance locally, be aware of these other steps/tips.
 
1. After cloning this repo to your local computer, you need to set up Snowflake creds. If you have Snowflake username/password, use `example_secrets.toml` template as in the default instructions. In many cases you don't have a Snowflake username/password so use `example_secrets_2.toml` template instead to use SSO login mechanism for the app.
2. Since we also integrate Google API data into Streamlit, you also need to obtain a Google creds file. *(Note: the app will run if you don't wish to set up it right away; Google API-related figures will simply not show data.)* Currently, this secret is accessible only to members of the GitHub nf-osi organization. Open Codespace on `main` and run: `echo $GOOGLE_CREDS | base64 -d > google_creds.json`. Save this file to your local computer at the root of the repo. ![image](https://github.com/user-attachments/assets/bc4c741f-1cf8-4aec-a717-38013bfc5b10)
...
> [!CAUTION]
> Do not commit your `secrets.toml` file to your forked repository. (It should already be listed in `.gitignore`.) Keep your credentials secure and do not expose them to the public.
...
Create a new Conda environment specifically for this project, with a compatible Python version (e.g., Python 3.10):
```bash
...
This will install Streamlit, pytest, and any other libraries required by the project.
```bash
pip install -r requirements.txt
```

### 4. Configure VSCode to Use the Conda Environment
Open the project folder in VSCode, then select the Conda environment as the Python interpreter:

1. Open the Command Palette by pressing `Ctrl+Shift+P` (Windows/Linux) or `Cmd+Shift+P` (Mac).
2. Type “Python: Select Interpreter” and select the `streamlit` environment you created.

### 5. Create An Active Workspace on VSCode
...
### 6. Adjust the `secrets.toml` File

You need to create a new `secrets.toml` file based on one of the two templates provided.

1. **For users with shared accounts (e.g., DPE group):** Use `example_secrets.toml`:
    ```toml
    [snowflake]
    user = "EXAMPLE_USER"
...
2. **For users without a shared account:** Use `example_secrets_2.toml`:
    ```toml
    [snowflake]
    name = "Vxxxxx"
    ...
    # SSO option instead of user/password -- suitable for local, non-container development
    user = "you@sagebase.org"
    authenticator = "externalbrowser"
    ```

Make sure to first log in to the web application and change your role from "PUBLIC" to "DATA_ANALYTICS" to gain access to the Synapse data warehouse.

### 7. Review The `launch.json` Configurations
